...
public class AdminHomeMainPresenter implements AdminHomeMainPresenterInterface,
		OnAdminHomeMain_FinishenListener {
	...
	public void validatecredential(String event, int p_id) {
		if (event.equals("start")) {
			interactor.getProgramInfo(view.getActivity(),p_id, this);
		} else {
			interactor.Renew(view.getActivity(),p_id, this);
		}
	...
	public void onSuccess(JsonObject result) {
		// TODO Auto-generated method stub
		Hashtable<String, ArrayList<Group>> groupstable = new Hashtable<>();
		...
		int p_is_available_overnight = (dataobj.get("p_is_available_overnight"))
				.getAsInt();
		...
		data = new ProgramData(p_id, p_name, p_info, e_name, p_cost_adult,
				p_cost_kid, p_min_persons, p_max_persons, p_available_season,
				p_lead_time, p_preparation_item, p_town_name, p_addr_1,
				p_addr_2, p_addr_3, p_addr_4, p_addr_5, p_addr_6, p_tel, p_url,
				p_is_available_overnight, p_photo_url, cur_list, p_note,
				p_grade, admin);
		...
		for (int i = 0; i < group.size(); i++) {
			...
			Group groupobject = new Group(g_id, p_id2, u_id, g_name, g_persons,
					...
					e_name2, p_name2, p_addr, p_photo_url2);
			groupList.add(groupobject);
			String key = String.valueOf(g_start_year * 10000)
					+ String.valueOf(g_start_month * 100)
					+ String.valueOf(g_start_day);
			if (groupstable.containsKey(key)) {
				groupstable.get(key).add(groupobject);
			} else {
				ArrayList<Group> newarray = new ArrayList<>();
				newarray.add(groupobject);
				groupstable.put(key, newarray);
			}
		}
		...
		for (int i = 0; i < json_afterword.size(); i++) {
			...
			afterword.add(new Afterword(a_id, g_id, u_id, a_content, a_grade,
					a_year, a_month, a_day, a_r_num, afterword_reply,
					a_photo_url, a_u_photo_url));
		}

		view.setFragment(data, groupList, afterword, groupstable);
	...
	public void onRenew(JsonObject result) {
		...
		view.renewFragment(data, groupList, afterword, groupstable);
