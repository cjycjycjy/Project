...
public class Fragment_JoinOne extends Fragment implements OnClickListener {
	...
	public void setActivityValue() {
		...
		String phone = edit_phone.getText().toString();
		((JoinActivity) getActivity()).setId(id);
		((JoinActivity) getActivity()).setPhone(phone);
		((JoinActivity) getActivity()).setPasswd(passwd);
	}

	public void confirmOverlapID() {
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					retofit.getConfirmOverlapID(id, new Callback<String>() {
						@Override
						...
						public void failure(RetrofitError retrofitError) {
							...
							if (getActivity() != null) {
								...
								Dialog_BaseDialog dialog = new Dialog_BaseDialog(
										getActivity(), getActivity().getString(
												R.string.failed_dialog));
								dialog.show();
							}
						}
					});
			...
			public void run() {
				try {
					...
					RestAdapter restAdapter = new RestAdapter.Builder()
							...
							.build();
					Retrofit retofit = restAdapter.create(Retrofit.class);
					retofit.getVerificationcode(phone,
									...
									if (getActivity() != null) {
										if (result.get("result").getAsString().equals("200")) {
											Dialog_BaseDialog dialog = new Dialog_BaseDialog(
													...
													"인증문자가 전송됩니다.\n잠시만 기다려주세요.");
											dialog.show();
											isconfirmedphone = true;// 인증번호 전송하면
											// 변경하기.
											confirmNumber = result.get(
													"verification_code")
													.getAsString();
										} else {
											Dialog_BaseDialog dialog = new Dialog_BaseDialog(
