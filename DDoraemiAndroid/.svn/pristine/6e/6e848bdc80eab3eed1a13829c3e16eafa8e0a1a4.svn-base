...
public class MyPagePresenter implements MyPagePresenterInterface,
	...
	public void validateCredentials(String event) {
		...
		{
			myPageInteractor.renewMyPageInfo(view.getActivity(), this,
					app.getId());
		}
	...
	public void OnGetDataSuccess(JsonObject data) {
		...
		for (int i = 0; i < wishArray.size(); i++) {
			...
			String p_note = "";
			if(!dataobj.get("p_note").isJsonNull())
				 p_note = (dataobj.get("p_note")).getAsString();
		...
		for (int i = 0; i < pastArray.size(); i++) {
			...
			if (dataobj.has("afterword")) {
				...
				afterword.put(temp.getG_id(), new Afterword(a_id, g_id, u_id, a_content, a_grade, a_year, a_month, a_day, a_r_num, afterword_reply, a_photo_url, app.getUser().getU_photo_url()));
			}
			pastgroup.add(temp);
		}

		view.setArrayList(particiategroup, wishprogram, pastgroup,afterword);

	}

	@Override
	public void OnGetDataSuccessForRenew(JsonObject data) {		// TODO Auto-generated method stub
		// //오브젝트를 지지고 볶은다음에
		particiategroup=new ArrayList<>(); 
		wishprogram=new ArrayList<>(); 
		pastgroup=new ArrayList<>();
		...
		for (int i = 0; i < pastArray.size(); i++) {
			...
			if (dataobj.has("afterword")) {
				...
				afterword.put(temp.getG_id(), new Afterword(a_id, g_id, u_id, a_content, a_grade, a_year, a_month, a_day, a_r_num, afterword_reply, a_photo_url, a_u_photo_url));
			}
			pastgroup.add(temp);
		}

		view.renewArrayList(particiategroup, wishprogram, pastgroup,afterword);
