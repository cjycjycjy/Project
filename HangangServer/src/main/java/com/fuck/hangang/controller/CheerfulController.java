...
public class CheerfulController {
	...
	public @ResponseBody <T> HashMap<String, T> getcheerfulone(@RequestParam("c_num") int c_num, @RequestParam("pm_num") int pm_num) {
		...
		if (resultCheerfulVo != null) {
			...
			cheerfulDao.updatePushisChecked(pm_num);
			resultHashMap.put("result", (T) "Success");
	...
	public @ResponseBody <T> HashMap<String, T> getcheerfulreplyone(@RequestParam("c_num") int c_num,
			@RequestParam("c_r_num") int c_r_num, @RequestParam("pm_num") int pm_num) {
	...
	public @ResponseBody <T> HashMap<String, T> registcheerfulreply(@RequestParam("c_num") int c_num,
		...
		try {
			...
			pushVo.setFlag(3); pushVo.setNum(c_num);
			if(!id.equalsIgnoreCase(u_id))
				cheerfulDao.registPushListAboutReply(pushVo);
			...
			if (push != 0) {
				if (!id.equalsIgnoreCase(u_id)) {
					...
					gs.sendGCMToOneUser(resultRegiId, c_num, 3, pushVo.getPm_num());
				}
			}
	...
	public @ResponseBody <T> HashMap<String, T> registcheerfulreplyofreply(@RequestParam("c_num") int c_num,
		...
		try {
			...
			String id = cheerfulDao.checkReplyOfReplyUser(c_num, c_r_num);
			

			PushVo pushVo = new PushVo();
			pushVo.setU_id(id); pushVo.setReply_writer(u_id);
			pushVo.setFlag(4); pushVo.setNum(c_num); pushVo.setReply_num(c_r_num);

			
			if(!id.equalsIgnoreCase(u_id))
				cheerfulDao.registPushListAboutReplyOfReply(pushVo);
			int push = cheerfulDao.checkPushUser(id);
			if (push != 0) {
				if (!id.equalsIgnoreCase(u_id)) {
					RegistrationIdVo resultRegiId = cheerfulDao.getRegiIdOfUserComment(c_r_num);
					GCMSender gs = new GCMSender();
					gs.sendGCMToOneUserAboutCommentOfComment(resultRegiId, c_num, c_r_num, 4, pushVo.getPm_num());
				}
			}
	...
	@RequestMapping(value = "/updatecheerful", method = RequestMethod.POST)
	public @ResponseBody <T> HashMap<String, T> updatecheerful(@RequestBody JSONObject json) {
		HashMap<String, T> resultHashMap = new HashMap<String, T>();
		CheerfulVo cheerfulVo = null;
		ObjectMapper mapper = new ObjectMapper();
		...
		try {
			...
			deleteFile(cheerfulDao.getcheerfulphotourl(cheerfulVo.getCh_num()));
			
			if (!cheerfulVo.getCh_photourl().equalsIgnoreCase("empty")) {
				writeFile(cheerfulVo.getCh_photourl(), savePath, cheerfulVo.getT_num(), time);
				ch_photo = filePath + cheerfulVo.getT_num() + "_" + time + ".jpg";
			}
			cheerfulDao.updatecheerful(cheerfulVo.getCh_num(),cheerfulVo.getCh_content(),
					ch_photo);
		...
		resultHashMap.put("result", (T) "Success");
		resultHashMap.put("photourl", (T)ch_photo);
		return resultHashMap;
	}
