...
public class QnaController {
	...
	public @ResponseBody <T> HashMap<String, T> updateqna(@RequestParam("q_num") int q_num,
			@RequestParam("title") String title, @RequestParam("content") String content) {
		HashMap<String, T> resultHashMap = new HashMap<String, T>();

		try {
			qnaDao.updateqna(q_num, title, content);
			resultHashMap.put("result", (T) "Success");
			return resultHashMap;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public @ResponseBody <T> HashMap<String, T> getqnareplyone(@RequestParam("q_num") int q_num,
			@RequestParam("q_r_num") int q_r_num, @RequestParam("pm_num") int pm_num) {
		...
		if (resultQnaReplyVo != null) {
			...
			qnaDao.updatePushisChecked(pm_num);
			resultHashMap.put("result", (T) "Success");
	...
	public @ResponseBody <T> HashMap<String, T> getcheerfulone(@RequestParam("q_num") int q_num, @RequestParam("pm_num") int pm_num) {
	...
	public @ResponseBody <T> HashMap<String, T> registqnareplyofreply(@RequestParam("q_num") int q_num,
		...
		try {
			...
			String id = qnaDao.checkReplyOfReplyUser(q_num, q_r_num);
			
			PushVo pushVo = new PushVo();
			pushVo.setU_id(id); pushVo.setReply_writer(u_id);
			pushVo.setFlag(6); pushVo.setNum(q_num); pushVo.setReply_num(q_r_num);

			
			
			if(!id.equalsIgnoreCase(u_id))
				qnaDao.registPushListAboutReplyOfReply(pushVo);
			int push = qnaDao.checkPushUser(id);
			if (push != 0) {
				if (!id.equalsIgnoreCase(u_id)) {
					RegistrationIdVo resultRegiId = qnaDao.getRegiIdOfUserComment(q_r_num);
					GCMSender gs = new GCMSender();
					gs.sendGCMToOneUserAboutCommentOfComment(resultRegiId, q_num, q_r_num, 6, pushVo.getPm_num());
				}
			}
			return resultHashMap;
		} catch (Exception e) {
